#!/bin/bash
# Start the frozen text8 Wave 2 exactly once and return.
set -euo pipefail

project_root=$(cd "$(dirname "$0")/.." && pwd)
cd "$project_root"

root=output/memory2_text8_wave2_20260810
...
nohup env \
    ...
    caffeinate -i bash scripts/run_memory2_text8_wave2_local.sh \
    > "$log" 2>&1 < /dev/null &
