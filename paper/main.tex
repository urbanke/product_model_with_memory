...
\emph{The conditional.}  At a fixed depth,
\begin{equation}
\label{eq:posterior-mean}
  ...
  \frac{\displaystyle\int \theta_y
       ...
       {\displaystyle\int
        \prod_{s=1}^{t} \theta_{x_s} \; \pi_L(d\theta)},
\end{equation}
where both integrals are over the vector $\theta$ with respect to
$\pi_L$ and the numerator carries one extra factor $\theta_y$.  This
is a weighted average of the coordinate $\theta_y$: each $\theta$
enters with weight proportional to $\prod_{s\le t}\theta_{x_s}$, the
probability that this $\theta$ gave to the prefix.  At $L = 1$ the
integrals are explicit and \eqref{eq:posterior-mean} equals
