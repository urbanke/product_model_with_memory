...
The results in this section are preliminary --- one file, the reduced
alphabet, two depths --- and are recorded so the numbers have a home;
more depths and files follow.
...
and the recursion given there, and the layered estimator at every
node.  Because every node's estimator is exchangeable, the whole
codelength is computed in closed form --- with the table of
Appendix~\ref{app:store}, the depth-3 run below took $80$ seconds.

\begin{table}[ht]
\centering
\begin{tabular}{lrrrrrr}
\toprule
run & $d{=}0$ & $d{=}1$ & $d{=}2$ & $d{=}3$ & family & leaves by depth \\
\midrule
$D=2$, layered & $6.0636$ & $5.0564$ & $5.1309$ & --- & $4.9507$ &
  $967 / 26{,}506$ \\
$D=3$, layered & $6.0636$ & $5.0564$ & $5.1306$ & $5.9071$ &
  $\mathbf{4.9390}$ & $969 / 21{,}817 / 17{,}698$ \\
$D=3$, KT leaves & $6.0636$ & $5.1123$ & $5.6164$ & $6.7815$ &
  $5.0825$ & $1{,}020 / 1{,}827 / 810$ \\
\bottomrule
\end{tabular}
\caption{Context trees on the reduced \texttt{text8} stream,
  $V = 1024$, bits per token.  The $d{=}k$ columns are complete
  fixed-depth trees; ``family'' is the mixture over all prunings;
  ``leaves by depth'' describes the pruning the posterior singles
  out.  The last row replaces the layered node estimator by the
  classical Krichevsky--Trofimov rule.}
\label{tab:ctree-v1024}
\end{table}

Three observations.  First, every complete fixed depth beyond one
\emph{loses} --- depth $2$ by $0.074$, depth $3$ by $0.85$ --- yet
the family beats the best fixed depth by $0.117$: all of the gain
of depth comes from choosing it per context.  Second, the gains
collapse as depth grows: allowing depth $2$ bought $0.106$ bits per
token, allowing depth $3$ buys another $0.0117$, although the chosen
pruning uses depth $3$ at $17{,}698$ contexts.  Memory of this kind
saturates quickly, as the count profile predicts: most deep contexts
are seen once or twice.  Third, the node estimator sets how much
memory the data can afford: with KT nodes the same family affords
almost no depth and lands $0.14$ above the layered version's
depth-one figure.
