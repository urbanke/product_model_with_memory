...
context tree (Sec.~\ref{sec:ctree})       & $1.7503$ & $\mathbf{2.0223}$ & $\mathbf{1.6392}$ \\
pairwise constraint model (Sec.~\ref{sec:pairwise}) & $\mathbf{1.7086}$ & pending & pending \\
\midrule
...
the current \texttt{text8} measurements; the two remaining corpora have not
yet been run.
...
\caption{Estimated continuous-update potential on the complete
  \texttt{text8} stream.  Each row uses 64 paired stratified prefix positions
