...
pairwise order two (Sec.~\ref{sec:pairwise}) & \multicolumn{3}{c}{campaign in progress} \\
\midrule
...
target--lag cells.  For a fixed observed context pair, its normalizer
is the untouched baseline mass plus a log-sum-exp over the union of
the two sparse correction lists.  The same identity computes fitted
margins and predictions.  Consequently neither a pair-probability
table nor a triple table is required by the repeated calibration
sweeps.  Active cells are matched individually; all inactive cells in
one lag row form a single grouped constraint.  This grouping, together
with restriction of the context support, defines the scalable model
being measured here; it is not claimed to equal the uncoarsened
full-support triangle.
