...
calibrated order two (Sec.~\ref{app:calibration}) & $1.9721$ & \multicolumn{2}{c}{campaign in progress} \\
\midrule
...
\item \emph{Product}: $m(y) \prod_{\delta \in \{1,2\}}
  ...
  target--lag pairs (Appendix~\ref{app:calibration}); tempered
...
tokens.  The reduced alphabet has $V=1024$: the $1023$ most frequent
token types in the file and one escape symbol.  This whole-file choice
is not free.  We transmit the selected subset with an enumerative code,
charge the external tokenizer vocabulary, and encode every original
token behind escape with a sequential Jeffreys--KT code over the
excluded tokenizer IDs.  The first $2050$ reduced symbols, before the
first fitted checkpoint, are likewise encoded by a fixed-alphabet KT
code.  Hence the final column below is a complete code for the original
character file, not a reduced-alphabet score.

\begin{table}[ht]
\centering
\caption{Calibrated pairwise prediction on full text8 at $V=1024$.
The middle column is diagnostic bits per BPE token.  The last column
includes the initial prefix, tokenizer and selected-vocabulary
descriptions, and escaped-token identities, and is in bits per original
character.}
\label{tab:calibrated-full-text8}
\begin{tabular}{lrr}
\toprule
predictor & reduced bits/token & complete bits/character \\
\midrule
two-pair star rule, \eqref{eq:star} & 5.17342 & 1.98367 \\
three-pair calibration, \eqref{eq:calibrated} & 5.06203 & 1.96203 \\
\midrule
gain from the third pair & 0.11139 & 0.02164 \\
\bottomrule
\end{tabular}
\end{table}

For the calibrated row, the complete $1.962025$ bits per character
decompose as follows:
\begin{center}
\begin{tabular}{lr}
\toprule
component & bits/character \\
\midrule
reduced-alphabet prediction, including initial prefix & 0.983516 \\
tokenizer vocabulary and selected-subset description & 0.062164 \\
identities of escaped tokens & 0.916345 \\
\midrule
total & 1.962025 \\
\bottomrule
\end{tabular}
\end{center}
The selected subset itself costs only $8{,}229$ bits; almost all of
the second line is the $6{,}208{,}152$-bit external-tokenizer charge.
The escape payload is $0.02087$ bits per BPE token above its empirical
entropy lower bound.  With 32 geometric checkpoints, calibration improves
all 31 predicted blocks, including the data-poor first block.  The complete
number is $0.010036$ bits per character below the earlier eight-checkpoint
measurement ($1.972062$); the denser causal refresh schedule, rather than a
change in vocabulary accounting, produces the improvement.
...
grouped margin error below $0.01$ without fallback or nonfinite rejection.
Reusable checkpoint construction took $871$ seconds with 12 processes;
calibration alone took $142.5$ seconds with 12 workers, versus $813$ seconds
for the earlier eight-checkpoint exact run.  Its initial implementation
peaked at $10.25$ GB.  A subsequent direct two-pass C builder eliminated the
temporary sparse matrices used to construct the intersection plan, and a
context-sparse reference cache retained only corrections used by each edge
block.  A complete validation run reproduced every fitted array and every
certificate exactly, took $151.3$ seconds, and reduced measured peak RSS to
$5.12$ GB.  At the final checkpoint the problem arrays occupy $29$ MB, the
$83{,}702{,}774$-entry compact intersection plan $1.34$ GB, and the compact
reference cache $0.776$ GB.  Exact predictive scoring of the 31 independent
checkpoint intervals is also process-parallel: four workers reproduce every
serial block and accounting total exactly in $87.7$ rather than $256.8$
seconds, a $2.93$-fold elapsed-time speedup.

\paragraph{Provisional $V=4096$ scaling run.}
To measure the next computational regime, rather than to set a final
codelength, we repeated the full-file fit at $V=4096$ with eight
checkpoints.  A prefix of $2050$ tokens is too sparse for this alphabet;
the first fit was therefore delayed to $16V=65{,}536$ tokens and the
earlier symbols remain covered by the initial sequential code.  All eight
reported fits have marginal certificate below $0.01$.  Construction took
$811$ seconds, calibration $4801$ seconds, and the complete fit $5621$
seconds ($93.7$ minutes); peak resident memory was $17.08$ GB.  The final
checkpoint contains $1{,}399{,}764$ observed context edges, the same number
of active first-lag corrections, and $2{,}288{,}630$ second-lag
corrections.  These are deliberately not presented as final compression
numbers: eight checkpoints make the predictor stale over long blocks, and
delaying the first fit also costs prediction.  The run establishes that
$V=4096$ is feasible on the laptop and that calibration, at $86\%$ of fit
construction-plus-calibration time, is now the dominant scaling problem.
At the largest checkpoints utilization also becomes memory-bandwidth
limited, and exploratory dual steps can still trigger cancellation in the
intersection identity even though the retained solutions are finite and
certified.  The production target remains the full vocabulary with 32
checkpoints, preferably with a progressive early vocabulary and cached
checkpoint profiles.

That 32-checkpoint $V=4096$ target has now been run on the full file using
...
plan occupies $4.48$ GB and the stochastic reference cache $3.19$ GB.  The
honest two-part code is $1.861324$ bits per character: $6.928956$ bits per
BPE token for reduced-alphabet prediction, $0.320795$ for vocabulary
description, and $2.330236$ for escaped-token identities.  The calibrated
suffix is $6.923955$ versus $7.094082$ bits per token for the star fallback,
a pure three-pair gain of $0.170127$.  These remain provisional
tolerance-$0.01$ figures, but establish both a substantial predictive gain
over $V=1024$ and laptop feasibility at the denser refresh schedule.
