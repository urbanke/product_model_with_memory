...
The family is the measured instance of the hierarchy route of
Appendix~\ref{app:routes}: condition each position on its last $k$
symbols, where $k$ is not fixed but chosen per context by a mixture
over all prunings of the context tree up to depth $D$, with the prior
and recursion given there and the layered estimator at every node.
Because every node's estimator is exchangeable, the codelength is
computed in closed form; with the table of Appendix~\ref{app:store},
every run below took about two minutes, so the family has now been
measured across both axes --- depth at a fixed alphabet, and alphabet
...
\bottomrule
\end{tabular}
\caption{Depth ladder on the reduced \texttt{text8} stream at
  $V = 1024$, bits per token.  ``Gain'' is best fixed depth minus
  family; the last column is the family's improvement over the
  family one depth shallower.  Each successive depth buys roughly a
  tenth of the previous one: the memory this axis can reach
  saturates at two to three symbols.}
...
\bottomrule
\end{tabular}
\caption{Alphabet trend, \texttt{text8}, bits per token, corrected
  evaluator throughout.  The adaptive gain shrinks by roughly a
  factor of three for every fourfold growth of the alphabet, and at
  the full vocabulary the family \emph{loses} to fixed depth one:
  the prior cost of describing the pruning exceeds what depth earns.
  Consistency checks: the depth-one baseline at $V=4096$ agrees with
  the independently verified state-family figure ($6.7132$), and the
  $V=16{,}384$ family reproduces the July cluster run to the fourth
  decimal.  The full-vocabulary figure supersedes the earlier
  published $10.0919$, which carried a small error from the
  since-repaired evaluator and table cache.}
\label{tab:ctree-vocab}
\end{table}

Three conclusions.  First, all of depth's value comes from choosing
it per context: every \emph{complete} fixed depth beyond one loses,
at every alphabet, yet the mixture beats the best fixed depth
wherever the alphabet is small enough.  Second, both axes exhaust
quickly.  Along depth the gains collapse geometrically
(Table~\ref{tab:ctree-depth}); along the alphabet they shrink toward
zero and cross it at the full vocabulary
(Table~\ref{tab:ctree-vocab}).  The count profiles say why: at large
$V$, or beyond depth two, almost every context is seen once or
twice, and a node with two observations cannot pay for its own
description.  Third, the node estimator remains decisive --- with
Krichevsky--Trofimov nodes in place of layered ones the family
affords almost no depth at any size (Section~\ref{sec:ctree}
ablations at $V=1024$: family $5.0825$, a tenth of the layered
splits).  Taken together: the tree is the right way to \emph{spend}
memory of a few symbols, and no way to acquire more.  Longer reach
must come from the other routes of Appendix~\ref{app:routes} ---
the constraint models of Section~\ref{sec:pairwise}, whose cost
grows with $V^2$ rather than $V^{k}$, and retrieval, whose cost does
not depend on $V$ at all.
...
calibrated member, however, failed numerically on all three of these
runs --- an overflow in the calibration fitting at large counts,
under repair --- so the one comparison they were meant to extend,
calibrated against the rest at scale, is still open.  The $V=4096$
counts round is running and will be reported here.
