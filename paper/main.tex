...
\paragraph{Forecasts chose what to run; what is reported is measured.}
Each run above costs hours, so candidate grids were ranked beforehand
by a forecast that needs no model: count the states a map creates,
take the plug-in conditional entropy, and add the learning cost that
Table~\ref{tab:excess-where} measures per bucket of state size.  The
forecast ranks maps reliably on a fixed stream (within $0.0004$ bits
per token at moderate splits) but not across stream lengths (off by
$0.068$ bits per token on the $100$-million-token prefix, seven times
the effect being forecast), so it was used for the ranking and for
nothing else.  Every number in Tables~\ref{tab:firstorder}
and~\ref{tab:order-two} is measured.
