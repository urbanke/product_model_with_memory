...
\paragraph{A code is a probability assignment.}  A lossless code and a
probability assignment are the same object seen from two sides.  If
$Q$ gives every length-$n$ sequence a nonnegative probability and
these sum to one, then arithmetic coding turns $Q$ into a code that
spends $-\log_2 Q(x^n)$ bits on the file $x^n$, up to $O(1/n)$; and
any code's lengths define such a $Q$.  Choosing a model therefore
means choosing $Q$: a rule, fixed in advance and known to the decoder,
that spreads one unit of probability over all $d^n$ candidate files.
A rule is good if it concentrates its probability on files like the
ones we actually meet.  Every number reported in this paper is
$-\tfrac1n \log_2 Q(x^n)$ for some such rule, evaluated on the actual
file.

The rule used throughout is the layered product-simplex construction
of~\cite{layered2026}.  It is built in three steps: a weighting over
the simplex, an assignment at each depth, and an average over depths.
...
$\pi_L$ on the simplex --- our choice of weighting, not a statement
about the data.  At $L=1$ it is uniform; as $L$ grows it concentrates
on increasingly sparse, low-entropy vectors.  The depth thus
parametrizes a family of weightings ranging from ``all $d$ symbols
comparably likely'' to ``a few symbols carry almost all the mass''.

\emph{The assignment at depth $L$.}  A point $\theta$ of the simplex
assigns to a string the product $\prod_{s \le t} \theta_{x_s}$ ---
the probability that a memoryless source with parameters $\theta$
would give it.  The file is not assumed to come from any such source;
the product is a building block and nothing more.  The depth-$L$
assignment is its average under the weighting $\pi_L$, defined for
\emph{every} finite string at once:
\begin{equation}
\label{eq:mixture}
  Q^{(L)}(x^t)
  \;=\;
...
\end{equation}
Restricted to the $d^t$ strings of any one length this sums to one
(sum the product over strings inside the integral), so it is a
probability assignment at every length.  And since the product
depends on the string only through its counts, so does $Q^{(L)}$: the
assignment is \emph{exchangeable}.

\emph{The average over depths.}  The depth-averaged assignment
\begin{equation}
...
\end{equation}
commits to no single depth, and is again defined on every finite
string at once.

\paragraph{Why this is a sequential code.}  A decoder reconstructs
the file one symbol at a time, so at step $t$ it needs the
probability of each candidate next symbol, computed from the symbols
it has already recovered.  Three facts govern this, and they are
worth separating, because two of them hold and the plausible third is
false.

\emph{First: the values at different lengths fit together.}  For any
prefix $x^t$ and symbol $y$,
\begin{equation}
\label{eq:consistency}
  \sum_{y} Q\bigl(x^{t}y\bigr) \;=\; Q\bigl(x^{t}\bigr),
\end{equation}
for $Q = Q^{(L)}$ and hence for $Q = Q^{\mathrm{avg}}$.  At a fixed
depth this is one line: summing $\theta_y$ over $y$ inside the
integral \eqref{eq:mixture} gives $1$.  The average inherits it
because \eqref{eq:consistency} is a sum, and sums pass through the
average over depths unchanged.  So the conditional
\begin{equation*}
  Q(y \mid x^{t}) \;=\; \frac{Q(x^{t}y)}{Q(x^{t})}
\end{equation*}
is a genuine probability distribution over $y$ at every step, and
both its ingredients depend only on symbols already decoded.

\emph{Second: what the conditional is.}  At a fixed depth, writing
the ratio out,
\begin{equation}
\label{eq:posterior-mean}
  Q^{(L)}(y \mid x^{t})
  \;=\;
  \frac{\displaystyle\int \theta_y \,
        \prod_{s=1}^{t} \theta_{x_s} \; \pi_L(d\theta)}
       {\displaystyle\int
        \prod_{s=1}^{t} \theta_{x_s} \; \pi_L(d\theta)}
\end{equation}
--- an average of $\theta_y$ over the simplex in which each candidate
$\theta$ is weighted by $\prod_{s\le t}\theta_{x_s}$, the probability
it assigned to the prefix.  A $\theta$ that has explained the data
well counts for more; the weighting $\pi_L$ alone decides nothing
once data have arrived.  (In Bayesian language,
\eqref{eq:posterior-mean} is the posterior mean of $\theta_y$.)  At
$L=1$ the integrals are explicit and \eqref{eq:posterior-mean} is
Laplace's rule $(m_y + 1)/(t + d)$, with $m_y$ the number of
occurrences of $y$ in $x^t$.

For the depth average, define the weight that the prefix has earned
for depth $L$,
\begin{equation}
...
\end{equation}
the analogue over depths of the reweighting over $\theta$ above.
Expanding the ratio,
\begin{equation*}
  Q^{\mathrm{avg}}(y \mid x^{t})
  \;=\; \sum_{L=1}^{L_{\max}} w_L(x^{t})\, Q^{(L)}(y \mid x^{t}).
\end{equation*}
The prediction the decoder runs is therefore an average over depths
of averages over parameters, and at both levels the weights are
earned from the decoded prefix; the uniform $1/L_{\max}$ and the
weighting $\pi_L$ never appear in any single prediction.

\emph{Third, the false one: the conditional of the average is not
the uniform average of the conditionals.}  A ratio is not a sum, and
averages do not commute with ratios; the correct weights are
\eqref{eq:depth-posterior} and they move at every step.  A receiver
that instead kept the depth weights uniform at every step would also
be a valid code --- but a different one, and a worse one, and its
cost could not be computed from final counts at all
(Appendix~\ref{app:factorisation}).

\emph{One evaluation at the final counts.}  The posterior-weighted
receiver never has to be run step by step: consecutive ratios
telescope, so its total cost is $-\log_2 Q^{\mathrm{avg}}(x^n)$,
one evaluation at the end of the file.  By exchangeability this
depends on $x^n$ only through its count profile $\lambda$ (the
multiset of nonzero counts):
...
expression are the up-front weights on whole-sequence probabilities;
every step's earned weight has cancelled against the member
probability it was built from.  Appendix~\ref{app:factorisation}
proves the same statement per state for the models with memory,
where it carries the whole construction.
...
the file itself, where it equals $\hat H_n$.  So no fixed
per-symbol assignment can beat $\hat H_n$.  But no decoder can
\emph{achieve} it either: the decoder cannot know the frequencies of
data it has not yet received.  A real scheme closes this gap in one
of two ways.  Either send the frequencies first and then code the
data under them, paying for their description; or use at each step
only probabilities computable from the symbols already decoded ---
as every scheme in this paper does --- paying instead through the
early predictions, which are made from few observations.  The excess
...
role in any codelength --- the loss is always that of the full
average \eqref{eq:depth-average} --- but it is an interpretable
diagnostic: it names the single depth that best explains the data,
and its position (relative to $\ln d$) locates the source on the
