...
\title{A Product Model for Sources with Memory\\[2pt]
  \large Experimental log}
\author{}
\date{\today}
...
\begin{abstract}
This document records the experiments of the \texttt{product\_model\_with\_memory}
project, which extends the layered product-simplex predictor
of~\cite{layered2026} from exchangeable (memoryless) sources to sources with
memory.  The past is summarized in a state, and the scheme predicts from
(state, next token) pairs; the simplest instance is a first-order Markov
model in which the state is the previous token.  The document is a running
log: sections are added as experiments complete, and every number is
produced by a script in the repository.
\end{abstract}
...
\end{equation}
It is an in-sample quantity, and it is worth being precise about the
sense in which it is a floor.  An arithmetic code driven by a fixed
probability assignment $p$ on the alphabet spends
$-\tfrac1n\sum_t \log_2 p(x_t)$ bits per token; over all constant
(i.i.d.) assignments this is minimized at $p_i = m_i/n$, the relative
frequencies of the prefix itself, and the minimum value is $\hat H_n$.
So no constant assignment can do better than $\hat H_n$ on this prefix.
But $\hat H_n$ is not itself the length of a code.  Decoding an
arithmetic code requires the decoder to use the same probabilities as
the encoder, and while the encoder can compute $m_i/n$ --- it has the
whole prefix in front of it --- the decoder cannot: the relative
frequencies are a function of exactly the data it has not yet received.
A decodable scheme must close this gap in one of two ways.  Either
transmit the frequencies first and then code the data under them (a
two-part code, which pays the description cost of the frequencies), or
use at step $t$ only probabilities computable from $x^{t-1}$, which
encoder and decoder both know at that point (a sequential code, which
pays instead through its early predictions, made from few
observations).  All codelengths reported in this paper are of the
second kind.  $\hat H_n$ is thus the floor against which any memoryless
estimator is measured, and the excess above it is the cost of closing
this gap.
...
\end{equation}
the two forms being equal by the chain rule; by the Kraft inequality this
is (up to $O(1/n)$) the length of an actual arithmetic code, so
$\ell_n(Q)$ is realizable, not merely a score.  The \emph{redundancy} we
tabulate is $\ell_n(Q) - \hat H_n$: the part of the codelength paid for
\emph{learning} the distribution rather than describing the data under a
known one.

For the mixtures used here the sequential product in
\eqref{eq:codelength} never needs to be accumulated term by term: the
mixture is exchangeable, so $Q(x^n)$ depends on $x^n$ only through its
count profile $\lambda$ (the multiset of nonzero counts), and
$-\log_2 Q^{\mathrm{avg}}(x^n) =
-\log_2\bigl[\tfrac1{L_{\max}}\sum_L q_\lambda(L)\bigr]$, where
$q_\lambda(L)$ is evaluated by the layer recursion and saddlepoint method
of Appendix~B of~\cite{layered2026}, ported to this repository
(\texttt{src/product\_model\_with\_memory/layered.py}, with the batched
table builder of \texttt{fast\_tables.py} validated against the
original).
...
\section{Validation: reproducing the King James Bible experiment}
\label{sec:kjv}

Before any new experiment we re-run the real-text experiment of
\cite{layered2026}, Section~5.3 (Table~10) with the ported code.  The paper
used Project Gutenberg etext \#10 ($915{,}849$ word-and-punctuation tokens,
$13{,}550$ distinct types); our corpus is the closely related eBook \#30
(``The Bible, King James Version, Complete''), which after removal of the
header, verse markers, and book headings tokenizes to $917{,}150$ tokens and
$13{,}542$ types (a $0.15\%$ edition difference).  Alphabet $d=100{,}000$,
$L_{\max}=\operatorname{round}(2\cstar\ln d)=54$.
Table~\ref{tab:kjv} compares the published redundancies of the
depth-averaged predictor with our reproduction.

\begin{table}[ht]
\centering
\begin{tabular}{rrrrrrr}
\toprule
$n$ & $\hat H_n$ (paper/ours) & \multicolumn{2}{c}{redundancy PS avg} & \multicolumn{2}{c}{mode $L$} \\
 & & paper & ours & paper & ours \\
\midrule
 10{,}000 & 7.56 / 7.504 & 1.052 & 1.019 & 21 & 22 \\
 30{,}000 & 7.87 / 7.844 & 0.598 & 0.584 & 21 & 21 \\
100{,}000 & 8.02 / 8.013 & 0.306 & 0.302 & 21 & 21 \\
300{,}000 & 8.18 / 8.182 & 0.168 & 0.167 & 19 & 19 \\
full text & 8.48 / 8.478 & 0.095 & 0.095 & 15 & 15 \\
\bottomrule
\end{tabular}
\caption{Reproduction of the King James Bible experiment
  (\cite{layered2026}, Table~10) with the ported code
  (\texttt{scripts/unigram\_experiment.py}) on eBook \#30.  Redundancy in
  bits/token of the depth-averaged product-simplex predictor.  Agreement is
  to the third decimal on the full text and within the differences
  expected from the two editions at the smallest prefixes (at $n=10^4$ the two corpora differ most in
  relative terms: $1{,}108$ vs.\ $1{,}161$ observed types); the posterior
  depth sequence $21\!-\!22 \to 15$ is reproduced.  We regard the port as
  validated.}
\label{tab:kjv}
\end{table}

\section{The text8 corpus: memoryless baseline}
\label{sec:text8}

The first corpus beyond the Bible is \texttt{text8}: the first $10^8$ bytes
of an English Wikipedia dump, stripped to lowercase a--z and space
($17{,}005{,}207$ tokens, $253{,}854$ distinct types; canonical file, md5
\texttt{3bea1919949baf155f99411df5fada7e}).  Empirical unigram entropy of
the full corpus: $10.856$ bits/token; empirical conditional entropy given
the previous token: $7.286$ bits/token, so first-order memory is worth about
$3.57$ bits/token in sample.  We take $d=2^{18}=262{,}144$ (the smallest
power of two above the number of observed types), giving
$L_{\max}=59$.

The column ``mode $L$'' in Table~\ref{tab:text8-unigram} is the mode
$L^\star$ of the depth posterior \eqref{eq:depth-posterior}: among the
$L_{\max}=59$ single-depth mixtures $Q^{(L)}$ that make up the average
\eqref{eq:depth-average}, it is the depth to which the data assign the
largest posterior weight $w_L \propto Q^{(L)}(x^n)$ --- equivalently, the
depth whose mixture gives the prefix the shortest codelength.  It is a
reported quantity, not a choice: every codelength in the table is that of the full
depth average, in which all $59$ depths participate with their posterior
weights.  Watching $L^\star$ move is nonetheless informative, and
Table~\ref{tab:text8-unigram} shows a pronounced drift discussed in the
caption.
...
\toprule
$n$ & types & $\hat H_n$ & PS avg & redundancy & mode $L$ \\
\midrule
     10{,}000 &   2{,}520 &  9.275 & 11.462 & 2.187 & 15 \\
     30{,}000 &   5{,}651 &  9.736 & 11.183 & 1.446 & 13 \\
    100{,}000 &  12{,}023 & 10.109 & 10.939 & 0.830 & 12 \\
    300{,}000 &  24{,}537 & 10.419 & 10.911 & 0.492 & 10 \\
 1{,}000{,}000 &  52{,}754 & 10.614 & 10.876 & 0.262 &  8 \\
 3{,}000{,}000 &  96{,}644 & 10.719 & 10.853 & 0.134 &  6 \\
10{,}000{,}000 & 189{,}074 & 10.852 & 10.910 & 0.058 &  4 \\
17{,}005{,}207 & 253{,}854 & 10.856 & 10.895 & 0.039 &  4 \\
\bottomrule
\end{tabular}
\caption{Memoryless (unigram) baseline on \texttt{text8}
  ($d=2^{18}$, $L_{\max}=59$; full run, 20 cores, 41 min): codelength per
  token of the depth-averaged predictor, redundancy above the empirical
  unigram entropy of the prefix, and the posterior-mode depth.  Two
  observations.  First, the redundancy decays smoothly to $0.039$
  bits/token on the full corpus.  Second, the selected depth falls from
  $L=15$ ($\approx 1.2\ln d$) to $L=4$ ($\approx 0.3\ln d$): unlike the
  Bible (13.5\% of a $10^5$ vocabulary used, depth settling at
  $1.3\ln d$), \texttt{text8} eventually reveals 97\% of its
  $2^{18}$-sized alphabet, the target is not sparse relative to $d$, and
  the posterior moves toward the shallow (Laplace-like) end.  This is
  the behaviour \cite{layered2026} predicts for targets that are not
  sparse relative to the alphabet, observed here on one corpus by
  varying $n$ alone.}
\label{tab:text8-unigram}
\end{table}

\section{Pairs: plug-in conditionals from a joint mixture}
\label{sec:pairs}
...
\caption{Pair experiment on full \texttt{text8} ($n = 17{,}005{,}207$
  ...
  as in Table~\ref{tab:text8-unigram}.  Predictive normalization
  diagnostics: $10^{-13}$--$10^{-6}$ in all rows.}
\label{tab:pairs}
\end{table}

\section{Program: averaging over a family of state maps}
\label{sec:states}
...
The first run of this family (full \texttt{text8}, $V=1{,}024$) uses the
ten-member grid
\[
...
aggregate.  The instruments that spend resolution selectively ---
intermediate $M$ (frequent words resolved, rare words pooled), and the
adaptive tree of the next subsection at this vocabulary --- are
exactly what the remaining $2.8$ bits/token of learning cost pay for;
the intermediate-$M$ members at full vocabulary are the natural next
run, and for the first time in the first-order family the optimum
could plausibly be interior.
...
\subsubsection*{First run: full \texttt{text8}, $V=1{,}024$, $D=2$}
...
\subsubsection*{Second run: full \texttt{text8}, $V=4{,}096$, $D=2$}
...
$L$ --- a distinction invisible at $V=1{,}024$ on short lags but, as
the second run shows, both visible and measurable at $V=4{,}096$.  Second, the $\delta = 1$
...
The second run (deltas $\{0,1,2,3,4,6,8,12,16,24,32\}$; $40{,}961$
...
after accounting for the overlap between lags has not been measured.
...
\end{figure}
\section{Comparison with published results on \texttt{text8}}
...
($1.60$), and uses no memory at all.  The first-order code at the full
vocabulary ($X = 10.0909$, Section~\ref{sec:state-family-endpoints})
gives $\mathbf{1.7503}$ bits/character --- the first full-fidelity
number with memory.  The tally so far: $1.8870$ memoryless, $1.7503$
first order; the remaining distance to \texttt{ppmd} is $0.15$ bpc.
...
and \texttt{paq8h} if it stays below $0.75$.  The \emph{complete}
first-order map is now measured and does not meet either threshold:
its learning cost is $2.805$ bits/token
(Section~\ref{sec:state-family-endpoints}).  The burden therefore
falls on structures that spend resolution selectively --- intermediate
$M$ (frequent words resolved, rare words pooled) and the adaptive tree
at full vocabulary, which can decline a successor row where it cannot
pay --- and on depth where it is affordable.  Two further reductions follow the two regimes of
...
\paragraph{Roadmap.}
The work is organized in four phases.  \emph{Phase 0} (done, this
section): the full-fidelity accounting --- escape mechanism, spelling
model, index correction --- giving the conversion above and the
$1.8870$ bpc memoryless figure.  \emph{Phase 1} (compute-bound; the
code exists): the state-family and context-tree experiments at
$V = 16{,}384$, $65{,}536$, and the full vocabulary; profile
evaluations are independent, so the work parallelizes over cluster
nodes with a shared moment-table cache.  Alongside: the KT-leaf
version of the context tree (same tree, KT estimators in place of
layered mixtures), which isolates the contribution of the layered
prior, and depth $3$.  Expected outcome: a full-fidelity first-order
and depth-$2$ figure; the targets for $X$ above say what the
learning cost must be for these to pass \texttt{ppmd} and
\texttt{paq8h}.  \emph{Phase 2} (code, then compute): the sequential
pooled evaluator --- per-step predictive distributions from
checkpointed expert tables, weight grids mixed at sequence level,
context-dependent weights, doubling distance bands --- with its test
suite (exact enumeration on a small alphabet; a one-hot weight
recovering the lag-1 member; the cost of checkpoint staleness
measured directly).  Its scientific output is the marginal gain per
added lag compared with $I(\delta)$, i.e.\ how much of the tail
survives the overlap between lags.  \emph{Phase 3}: one sequential
code containing all parts --- spelling, full-vocabulary adaptive
tree, gated lag pool --- reported both as whole-file bits/character
(comparable to Mahoney's table) and as bits on the last $5\%$ given
the first $95\%$ (comparable to the neural results).

\section{Outlook: generating state maps, not designing them}
\label{sec:outlook}

This section records the candidate schemes for state maps that
summarize \emph{much longer} contexts, together with the criterion that
selects them.  Throughout, two design principles are held fixed:
