...
\item \emph{References}: the lag-one predictor alone, and order-two
  states with backoff to lag one, the $V^3$-scale construction this
  section tries to approximate.
\end{itemize}

None of these predictors is exchangeable over any partition of the
positions, so the one-evaluation route of
Section~\ref{sec:firstorder} is closed.  The code is instead the
checkpointed one, as for the pooled lags: the stream is cut into $C$
equal blocks, every table and the calibration are computed from the
data strictly before the block, the block is coded, and the tables
are refreshed.  Every row of the comparison carries the same
staleness cost, so the comparison is internal, and each member is a
...
The measured comparison will be reported here when the runs
with the layered estimator complete.
