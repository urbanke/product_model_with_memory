...
calibrated order two (Sec.~\ref{sec:calibration}) & $1.8766$ ($V=4096$) & pending & pending \\
\midrule
...
Appendix~\ref{app:routes}.  Table~\ref{tab:calibrated-text8} reports
the first controlled full-file measurements; larger alphabets and the
two remaining corpora are still being run.
...
complete codes, and bits per character of the original file.  For a
reported alphabet size $V$, the $V-1$ retained token types and one
escape symbol form the reduced stream.  The table separately shows
the reduced-stream prediction, the external vocabulary and retained-
subset description, and the sequential KT payload behind escape.
These components sum to the honest full-stream rate.
...
\midrule
$1{,}024$ & $0.9923$ & $0.0622$ & $0.9163$ & $1.9708$ \\
$4{,}096$ & $1.3615$ & $0.0623$ & $0.4527$ & $1.8766$ \\
\bottomrule
\end{tabular}
\caption{Calibrated three-pair prediction on \texttt{text8} with the
  \texttt{cl100k\_base} representation, in bits per character of the
  original $10^8$-character file.  Both rows use $32$ geometrically
  spaced checkpoints, beginning after $65{,}536$ tokens.  The first
  three code components sum to the honest total (up to rounding).}
\label{tab:calibrated-text8}
\end{table}

For comparison under exactly the same accounting, the two-pair
maximum-entropy rule gives $1.9825$ and $1.8964$ bits per character at
$V=1{,}024$ and $V=4{,}096$, respectively.  Using only the most recent
token gives $1.9836$ and $1.8801$.  The calibrated rule therefore gains
$0.0117$ and $0.0198$ over the two-pair rule, and $0.0128$ and $0.0035$
over the one-pair rule.  At $V=4{,}096$ the third context pair is on
observed support for $91.78\%$ of the scored tokens; elsewhere the
declared two-pair fallback is used.  The fits use the fixed relaxed
calibration rule described in Appendix~\ref{app:calibration-implementation};
they define honest predictive distributions even when the finite-sample
pair margins are not exactly compatible.
...
Table~\ref{tab:calibrated-text8}.  Their implementation and complete-code
