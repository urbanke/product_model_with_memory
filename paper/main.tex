...
The question this section must answer is the paper's standing one:
does the construction move the best honest number of
Table~\ref{tab:firstorder} toward the published targets of
Table~\ref{tab:files}?  For \texttt{text8} the gap is $1.8298$
against $1.398$ bits per character.  The construction tried here is
the hierarchy route of Appendix~\ref{app:routes}: condition each
position on its last $k$ symbols, with $k$ chosen per context by an
exact mixture over all prunings of the context tree up to depth $D$,
the layered estimator at every node, and the whole codelength in
closed form via the table of Appendix~\ref{app:store}.

\paragraph{The verdict at full fidelity.}  At the full vocabulary
--- the only setting whose numbers are comparable to
Table~\ref{tab:firstorder} --- the answer is no.  The family over
all prunings up to depth two codes the \texttt{text8} stream at
$10.0926$ bits per token against $10.0915$ for its own depth-one
member alone: adaptive depth \emph{loses} $0.0011$ bits per token
($0.0002$ bits per character), because the price of describing the
chosen pruning exceeds what second-symbol memory earns when almost
every two-symbol context is seen once or twice.  Progress toward the
published targets from this axis: zero.  (This run also supersedes
an earlier version of the same number, $10.0919$, computed before
the evaluator and table repairs described in
Appendix~\ref{app:store}.)

\paragraph{Why measure it at all, then.}  Because \emph{where} depth
pays is exactly the information the promising routes need, and that
is measurable only in a controlled arena.  The remaining tables use
reduced alphabets --- the top $V$ tokens plus a catch-all --- and
their bits-per-token figures are honest codelengths \emph{of the
reduced stream}: internally comparable, deliberately not convertible
to bits per character of the file.  They are the mechanism study.

\begin{table}[ht]
\centering
\begin{tabular}{lrrrr}
\toprule
 & best fixed depth & family & gain & added by last depth \\
\midrule
$D=2$ & $5.0564$ & $4.9507$ & $0.1057$ & $0.1057$ \\
$D=3$ & $5.0564$ & $4.9390$ & $0.1174$ & $0.0117$ \\
$D=4$ & $5.0564$ & $4.9384$ & $0.1180$ & $0.0006$ \\
\bottomrule
\end{tabular}
\caption{Depth ladder, reduced \texttt{text8} stream, $V = 1024$,
  bits per token of the reduced stream (not comparable to
  Table~\ref{tab:firstorder}).  Each successive depth buys roughly a
  tenth of the previous one.}
\label{tab:ctree-depth}
\end{table}
...
\begin{tabular}{rrrrr}
\toprule
$V$ & depth & best fixed & family & gain \\
\midrule
$1{,}024$  & $4$ & $5.0564$  & $4.9384$  & $+0.1180$ \\
$4{,}096$  & $3$ & $6.7132$  & $6.6766$  & $+0.0366$ \\
$16{,}384$ & $2$ & $8.2518$  & $8.2382$  & $+0.0136$ \\
$253{,}855$ & $2$ & $10.0915$ & $10.0926$ & $-0.0011$ \\
\bottomrule
\end{tabular}
\caption{Alphabet trend on \texttt{text8}, bits per token of each
  reduced stream; only the last row is at full fidelity.  The
  adaptive gain shrinks by roughly a factor of three per fourfold of
  alphabet and crosses zero at the full vocabulary.  Consistency:
  the depth-one figure at $V = 4096$ matches the independently
  verified state-family value; the $V = 16{,}384$ family reproduces
  the July run to the fourth decimal.}
\label{tab:ctree-vocab}
\end{table}

What the arena establishes.  First, all of depth's value is in the
per-context choice: every complete fixed depth beyond one loses at
every alphabet, yet the mixture wins wherever the alphabet is small.
Second, both axes exhaust geometrically
(Tables~\ref{tab:ctree-depth} and~\ref{tab:ctree-vocab}), and the
count profiles say why: deep or wide, nearly every context is seen
once or twice, and two observations cannot pay for a node's
description.  Third, the roughly thirty thousand contexts where
depth \emph{does} pay at $V = 1024$ are the map of synergy --- the
places where two past symbols jointly say what neither says alone
--- and that map is precisely what the constraint models of
Section~\ref{sec:pairwise} cannot see and will need.  The tree, in
this program, is an instrument, not a compressor: memory that
reaches further at full fidelity must come from constraints (cost
$V^2$) and retrieval (cost independent of $V$),
Appendix~\ref{app:routes}.

\section{Memory of order two from pairwise statistics}
\label{sec:pairwise}

Section~\ref{sec:ordertwo} located the failure of order-two states on
\texttt{text8} and \texttt{enwik8}: not the ceiling but the learning
cost of one successor distribution per context pair.  This section
asks how much of the order-two ceiling can be reached while learning
only \emph{pairwise} objects.  The predictor sees
$(x_{t-1}, x_{t-2})$ and is built from the three pair distributions
$p(x_t, x_{t-1})$, $p(x_t, x_{t-2})$ and $p(x_{t-1}, x_{t-2})$, of
which the third is, by stationarity, the same table as the first.
That is roughly $3V^2$ numbers against the $V^3$ scale of order-two
states: a lower ceiling, bought at an order-one learning cost.

The pairwise tables do not determine the conditional
$p(x_t \mid x_{t-1}, x_{t-2})$; a combining rule must be chosen, and
the choice is the experiment.  All rules below consume the same
smoothed tables (each row shrunk toward the running unigram
distribution), so the comparison isolates the rule
(\texttt{scripts/pairwise\_experiment.py}):

\begin{itemize}
\item \emph{Mixture}: $w_0\, m(y) + w_1\, \hat p(y \mid x_{t-1})
  + w_2\, \hat p(y \mid x_{t-2})$ over a grid of weights, $m$ the
  unigram distribution.
\item \emph{Product}: $m(y) \prod_{\delta \in \{1,2\}}
  \bigl[\hat p(y \mid x_{t-\delta})/m(y)\bigr]^{\beta_\delta}$,
  normalized, over a grid of exponents.  The point
  $\beta_1 = \beta_2 = 1$ is not a tuning choice: it is the exact
  conditional of the maximum-entropy joint consistent with the two
  target pairs alone, and it is exact precisely when the two lags are
...
\item \emph{Calibrated product}: the conditional of the
  ...
  distributions.  It still factorizes over the lags, but its two
  factors are calibrated jointly, through the third table, so that
  redundancy between the lags is discounted automatically;
  Appendix~\ref{app:calibration} derives it and the fitting
  procedure.
\item \emph{References}: the lag-one predictor alone, and order-two
  states, the $V^3$-scale construction this section tries to
  approximate.  The order-two reference comes in two forms: successor
  counts with backoff to lag one, and the layered estimator applied
  per context pair, each pair carrying its own mixture.
\end{itemize}

With one exception, none of these predictors telescopes to a one-shot
evaluation, so the code is the checkpointed one, as for the pooled
lags: the stream is cut into $C$ equal blocks, every table and the
calibration are computed from the data strictly before the block, the
block is coded, and the tables are refreshed.  Every row of the
comparison carries the same staleness cost, so the comparison is
internal, and each member is a valid sequential code.

The exception is the layered order-two reference.  It is a mixture,
so its exact sequential code collapses as in
Section~\ref{sec:firstorder}: coding the stream with the per-pair
tables updated after every token costs one evaluation per context
pair at the final counts.  We report this exact number next to its
checkpointed version.  The difference between the two is the price of
refreshing the tables only $C$ times instead of at every token; if it
is large, $C$ blocks are too few for the comparison above to be
trusted.

With two lags the Chow--Liu alternative (keep the two highest-MI
edges of the triangle and use the resulting tree) is degenerate:
$\mathrm{MI}(x_t, x_{t-1})$ and $\mathrm{MI}(x_{t-1}, x_{t-2})$ are
the same number, so the tree drops the weaker lag entirely and
reduces to the lag-one predictor.  It is printed as a per-checkpoint
diagnostic and becomes a real member only when the lag set grows.

The measured comparison is reported in
Section~\ref{sec:pairwise-layered}.
\section{The pairwise rules with layered tables}
\label{sec:pairwise-layered}

The round of Section~\ref{sec:pairwise} estimated every pair table by
count smoothing.  This section repeats it with one change: every
table the rules consume --- the pair conditionals, the unigram, and
the calibration inputs --- is estimated by the layered mixture
instead.  Same file, same reduced alphabet ($V = 1024$), same
checkpoint schedule ($C = 32$), same members.
Table~\ref{tab:pairwise-layered} shows both rounds side by side.

\begin{table}[ht]
\centering
\begin{tabular}{lrr}
\toprule
member & smoothed counts & layered tables \\
\midrule
lag one alone                  & $5.3393$ & $5.3146$ \\
star product ($\beta = 1$)     & $5.3917$ & $5.3062$ \\
best mixture                   & $5.2976$ & $5.2902$ \\
best tempered product          & $5.2746$ & $5.2338$ \\
calibrated product             & $5.2827$ & $\mathbf{5.1983}$ \\
order two, backoff             & $5.2446$ & $5.2152$ \\
order two, layered, checkpointed & ---    & $5.4031$ \\
order two, layered, exact      & ---      & $5.2053$ \\
\bottomrule
\end{tabular}
\caption{Bits per token on the reduced \texttt{text8} stream,
  $V = 1024$, $C = 32$.  Left column: the round of
  Section~\ref{sec:pairwise}, all tables count-smoothed.  Right
  column: the same rules with every table estimated by the layered
  mixture.  The best member of each round is the family's posterior
  choice; in the layered round its posterior mass is $1.0000$.}
\label{tab:pairwise-layered}
\end{table}

Three findings.

First, better tables help every rule.  Each member improves, by
$0.007$ (best mixture) up to $0.086$ (star product).  The comparison
of combining rules was not an artifact of crude estimation.

Second, the ranking flips at the top.  With count-smoothed tables the
order-two reference with backoff was the best member and the
calibrated product third.  With layered tables the calibrated product
wins outright, at $5.1983$ with posterior mass $1.0$: the rule that
respects all three pair distributions, with no free parameter,
overtakes the $V^3$-scale reference it was meant to approximate.
This is the strongest measured support so far for the constraint
route of Appendix~\ref{app:routes}: once the statistics are well
estimated, the maximum-entropy construction on top of them is not a
cheap substitute but the winner.

Third, the exact-versus-checkpointed control did exactly the job it
was built for.  The layered per-pair reference looks disastrous when
checkpointed --- $5.4031$, below lag one --- and its exact sequential
form, computable in one pass because that member is a mixture
(Section~\ref{sec:pairwise}), costs $5.2053$.  The difference,
$0.198$ bits per token, is the price of refreshing that member's
tables only $32$ times instead of after every token.  So the member
is sound and the schedule was the problem; conclusions about members
of its kind must come from the exact form, or from a much finer
schedule.  Note also what the exact number does not change: the
calibrated product, which pays the staleness cost like every
checkpointed member, still beats the exact reference.

This is one file at one reduced vocabulary: a ranking round, not a
compression claim.  Both rounds have since been run on
\texttt{enwik8}, and the counts round on \texttt{enwik9} (same
$V = 1024$, same $C = 32$).  The ordering matches the
\texttt{text8} counts round: order two with backoff best ($4.3096$
and $3.7360$ under counts; $4.3017$ under layered tables on
\texttt{enwik8}), tempered products next, mixtures behind, lag one
last.  The layered per-pair reference is far less damaged by the
checkpoint schedule on \texttt{enwik8} ($4.4194$, against $5.4031$
on \texttt{text8}); its exact number there is pending.  The
overflow that had destroyed the calibrated member on these files has
since been repaired, and the first finite number is in: on the
\texttt{enwik8} counts round the calibrated product scores $4.4836$
--- behind the best tempered products ($4.4222$) rather than ahead
of them as on \texttt{text8} under layered tables.  One caveat
attaches: the fitter's residuals on \texttt{enwik8} stall near
$10^{-3}$ at every checkpoint, two orders above the \texttt{text8}
level, indicating that the three pair tables are not yet mutually
consistent there (the margin-matching step needs more rounds on this
file's heavier tail).  The calibrated figure is therefore a
conservatively-fitted lower bound on the construction, and the
comparison at scale stays provisional until the fit converges.  The
\texttt{enwik9} and $V=4096$ counts rounds are running and will be
reported here.
...
\paragraph{Computation.}  The potentials are fitted by iterative
proportional fitting: cycle through the three pairs, each time
rescaling one potential multiplicatively so that its pair margin of
the current model matches the given table.  The $V^3$ joint is never
materialized; each pair margin of the triangle model is one
$V \times V$ matrix product (for example the $(0,1)$ margin is
$\psi_{01} \odot (\psi_{02}\,\psi_{12}^{\top})$), so a sweep costs
three matrix products.  The procedure converges to the
maximum-entropy solution for strictly positive tables; the smoothed
estimates used in Section~\ref{sec:pairwise} are strictly positive
by construction.  In the sequential code the fit is warm-started
from the previous checkpoint, and the residual margin error is
reported as a diagnostic.  The implementation was checked against a
dense enumeration at small $V$: the fitted margins match to
$10^{-14}$, the conditional \eqref{eq:calibrated} matches the
conditional of the fitted joint exactly, and the copy-lag case
reproduces $p(x_0 \mid x_1)$ to $10^{-11}$ while the star rule errs
by $0.36$ in total variation.
...
(Appendix~\ref{app:calibration} proves the two-lag case).  Which
...
constraint set; Appendix~\ref{app:calibration} is its two-lag theory;
