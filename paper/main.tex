...
\subsubsection*{Third and fourth runs: $V=16{,}384$ and the full
vocabulary}
Two further scales, run on the laboratory cluster
(Table~\ref{tab:context-tree-large}), complete the vocabulary sweep at
...
complete depth $1$      & 8.2518  & & 10.0909 \\
complete depth $2$      & 9.8690  & & 13.0817 \\
\textbf{family}         & \textbf{8.2383} & & 10.0919 \\
...
\caption{The context-tree family at the two largest scales (full
  ...
  profiles; cluster runs).  At $V=16{,}384$ the pattern of
...
$1.8870$ bpc memoryless figure.  \emph{Phase 1} (largely done): the
vocabulary sweep of the state-family and context-tree experiments up
to the full vocabulary, run on the laboratory cluster, and the
KT-leaf ablation.  Outcome: the interior optimum
...
alphabets --- pooled-context trees and multi-tier backoff are the
remaining Phase-1 runs; depth $3$ awaits a disk-backed statistics
builder.  \emph{Phase 2} (code, then compute): the sequential
pooled evaluator --- per-step predictive distributions of the
\emph{layered} per-state mixtures, refreshed at checkpoints and held
fixed in between (a valid sequential code; the staleness cost is
measurable by varying the checkpoint count) --- pooled under both
rules, weighted sums and tempered products, with weight and exponent
grids averaged at sequence level.  Test suite: exact enumeration on a
small alphabet; the telescoping identity (a per-step-refreshed
memoryless expert must reproduce the layered codelength of the
unigram profile exactly); one-hot weights under either rule agreeing
with each other.  Its scientific output is the marginal gain per
added lag compared with $I(\delta)$, i.e.\ how much of the tail
survives the overlap between lags, and the data's choice between the
additive and multiplicative combination.  \emph{Phase 3}: one sequential
