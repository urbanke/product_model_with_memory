...
observations.  Given $\theta$, tokens are drawn i.i.d.\ from $\theta$:
each $x_t$ takes the value $i$ with probability $\theta_i$.  The
depth-$L$ predictor assigns to \emph{every} finite sequence
$x^t = (x_1, \dots, x_t)$, of any length, the probability
\begin{equation}
\label{eq:mixture}
  Q^{(L)}(x^t)
  \;=\;
...
\end{equation}
--- one formula for all lengths; restricted to the $d^t$ sequences of
any one length $t$ it is a normalized probability assignment.  Its
conditionals
...
$x^t$ only through the counts $m_1, \dots, m_d$.
...
\end{equation}
requires no depth selection, and is again defined on every finite
sequence at once.
...
one a decoder runs, and it rests on two facts about the averaged
predictor that deserve to be stated separately, because one of them
survives the averaging and the other does not.

\emph{First fact: the values of $Q$ at different lengths fit
together.}  The conditional in \eqref{eq:codelength} is the ratio
$Q(x^{t-1}y)/Q(x^{t-1})$, which evaluates $Q$ on strings of two
different lengths.  For this ratio to be the conditional of a single
probability law, the value on the shorter string must equal the total
probability of its continuations:
\begin{equation}
\label{eq:consistency}
  \sum_{y} Q\bigl(x^{t}y\bigr) \;=\; Q\bigl(x^{t}\bigr).
\end{equation}
This holds at each fixed depth because $\sum_y \theta_y = 1$ inside
the integral \eqref{eq:mixture}; and it holds for the average
\eqref{eq:depth-average} because \eqref{eq:consistency} is a
\emph{sum}, and summing over continuations passes through the average
over depths unchanged.  So $Q^{\mathrm{avg}}$ is one function on all
finite strings, the ratio is legitimate, and both of its ingredients
depend only on symbols the decoder has already recovered.  By the
Kraft inequality the resulting arithmetic code has length
$-\log_2 Q(x^n)$ up to $O(1/n)$, so $\ell_n(Q)$ is realizable, not
merely a score.  The \emph{redundancy} we tabulate is
$\ell_n(Q) - \hat H_n$: the part of the codelength paid for
\emph{learning} the distribution rather than describing the data under
a known one.

\emph{Second fact: the conditional does not survive the averaging.}
A ratio is not a sum, and averages do not commute with ratios: the
conditional of the average is \emph{not} the uniform average of the
per-depth conditionals.  Expanding the ratio,
\[
...
depend only on what it has already recovered.

The same structure repeats one level down.  At a fixed depth,
\eqref{eq:mixture} is itself an average --- over the parameter
$\theta$, with respect to the depth-$L$ prior $\pi_L$ --- and its
conditional is likewise not the prior average of $\theta_y$ but the
posterior one,
\[
  Q^{(L)}(y \mid x^{t-1})
  \;=\; \int \theta_y \, \pi_L\bigl(d\theta \mid x^{t-1}\bigr)
  \;=\; \E\bigl[\theta_y \mid x^{t-1}\bigr],
  \qquad
  \pi_L\bigl(d\theta \mid x^{t-1}\bigr)
  \;\propto\; \prod_{s<t} \theta_{x_s}\; \pi_L(d\theta),
\]
the prior reweighted by the data seen so far; at $L = 1$ this
posterior mean is Laplace's add-one rule, as noted after
\eqref{eq:mixture}.  So the predictor the decoder actually runs is an
average over depths of averages over parameters, both weighted by
posteriors that the decoded prefix determines.  The priors themselves
--- uniform over depths, $\pi_L$ over $\theta$ --- never appear in any
single prediction; they enter only through the whole-sequence
probabilities.  A receiver that instead kept the depth weights uniform
at every step would also be a valid code --- but a different one, and
a worse one, and its cost could not be computed from final counts at
all (Appendix~\ref{app:factorisation}).
